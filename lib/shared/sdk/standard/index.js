...
class StandardAdapter extends EventEmitter {
  ...
  scan (options = {}, callback) {
    ...
    drivelist.listAsync().map((drive) => {
      ...
      const isLocalSystem = drive.isSystem || !drive.isRemovable
      return !drive.error && (options.includeSystemDrives || !isLocalSystem)
    }).map((drive) => {
      drive.displayName = drive.mountpoints.length ?
        ( process.platform === 'win32' ? `${drive.mountpoints.map(x => x.path).join(', ')}` : drive.raw ) : drive.raw
      return drive
    }).then((drives) => {
